...
function resolvePiPackageRootFallback(): string {
	...
	for (const pkgName of piPackageNames) {
		try {
			const entryPoint = require.resolve(pkgName);
			...
			while (dir !== path.dirname(dir)) {
				...
				try {
					...
					if (piPackageNames.includes(pkg.name)) return dir;
				} catch {
			...
			// Package not available, try next name
		}
	}
	throw new Error("Could not resolve pi-coding-agent package root");
}
