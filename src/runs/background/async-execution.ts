...
const jitiCliPath: string | undefined = (() => {
	const candidates: Array<() => string> = [
		...
		() => {
			// Resolve jiti from the active Pi package root (bundled dependency)
			if (piPackageRoot) {
				const pkgJitiPath = path.join(
					piPackageRoot,
					"node_modules",
					"jiti",
					"lib",
				...
				if (fs.existsSync(pkgJitiPath)) return pkgJitiPath;
			}
			throw new Error("jiti not found in pi package root");
		},
		() =>
			path.join(
